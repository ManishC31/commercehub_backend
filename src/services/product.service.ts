...
export const getProductByName = async (db: DbClient, productName: string) => {
  const query = `select * from products where lower(name) = $1 limit 1`;
...
export const createProduct = async (db: DbClient, data: NewProductInput) => {
  const query = `insert into products (
    title, description, category_id, rating, material, care_instructions, fit, price, discount) values ($1, $2, $3, $4, $5, $6, $7, $8, $9) returning *`;
